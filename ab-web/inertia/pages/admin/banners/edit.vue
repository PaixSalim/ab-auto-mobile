<template>
  <div class="px-6 py-8">
    <div class="flex justify-between items-center mb-6">
      <h3 class="text-primary font-bold text-3xl">Modifier une Bannière</h3>
      <Link
    ...
    <div class="bg-white shadow-md rounded-lg p-6">
      <form @submit.prevent="submitForm" enctype="multipart/form-data">
        ...
        <!-- Image actuelle -->
        <div v-if="banner.image && !imagePreview" class="mt-6">
          <label class="block text-gray-700 text-sm font-bold mb-2">Image actuelle</label>
          <img :src="banner.image" alt="Image actuelle" class="max-w-full h-48 object-cover rounded border" />
        </div>

        <!-- Aperçu de la nouvelle image -->
        <div v-if="imagePreview" class="mt-6">
          <label class="block text-gray-700 text-sm font-bold mb-2">Aperçu de la nouvelle image</label>
        ...
        <div class="mt-8 flex justify-end gap-4">
          ...
          >
            ...
            <span v-else class="i-mdi-pencil"></span>
...
onMounted(() => {
  // Pré-remplir le formulaire avec les données de la bannière
  form.title = props.banner.title
  form.description = props.banner.description
  form.link = props.banner.link
})
